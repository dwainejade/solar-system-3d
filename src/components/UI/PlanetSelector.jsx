...
const MenuItem = ({
  ...
  return (
    ...
    >
      ...
      {hasSubmenu && (
        <div className={`planet-selector-submenu ${showSubmenu ? "visible" : ""}`} role='menu' aria-label={`${label} submenu`}>
          {children}
        </div>
      )}
    </div>
    ...
    if (isOpen) {
      if (activeCamera.type === "moon") {
        ...
        if (parentPlanet) {
          ...
          const moonIndex = moonsData[parentPlanet]?.findIndex(moon => moon.name === activeCamera.name);
          if (moonIndex !== -1) {
